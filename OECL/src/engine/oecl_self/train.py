...
from util.utility import AverageMeter, ProgressBar, Rot90


def execute_clr(loader, model, loss_func, optimizer, device, rank, epoch):
    """
    :param loader:
    :param model:
    :param loss_func:
    ...
    for i, (views_id, views_oe) in enumerate(zip(loader["id"], loader["oe"])):
        ...
        data_id1 = torch.cat([Rot90(dims=(2, 3), idx=i)(data_id1) for i in [0, 1]], dim=0)
        data_id2 = torch.cat([Rot90(dims=(2, 3), idx=i)(data_id2) for i in [0, 1]], dim=0)
        ...
        data_oe1 = torch.cat([Rot90(dims=(2, 3), idx=i)(data_oe1) for i in [2, 3]], dim=0)
        data_oe2 = torch.cat([Rot90(dims=(2, 3), idx=i)(data_oe2) for i in [2, 3]], dim=0)

        assert data_id1.size() == data_oe1.size()
...
def train(params, loader, model, loss_func, optimizer, scheduler, start_epoch, rank, device):
    ...
    with mlflow.start_run(run_name="{}".format(params["description"])) as run:
        ...
        for epoch in range(start_epoch, params["epochs"]):
            ...
            if rank == 0:
                print("~1 execute: {:.2f}".format(time.time() - init_time))
