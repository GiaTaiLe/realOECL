...
def get_data(params, train, category, trn, formatter="aug"):
    ...
    if params[dataset_type] == "imagenet30":
        ...
        raise ValueError("Dataset {} currently unavailable".format(params[dataset_name]))

    return dataset


def _get_data(data, dataset_type, dataset_name, sub_classes, train, f_trn):
    if dataset_type in ["dataset", "dataset_oe"]:
