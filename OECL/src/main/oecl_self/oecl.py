...
from engine.oecl_self.config import PathConfig
from engine.oecl_self.test import *
from engine.oecl_self.train import train
from engine.oecl_self.wrapper import *
from util.utility import set_random_seed

# parser
parser = argparse.ArgumentParser(description="SimCLR_OE")
...
def main(rank=local_rank):
    ...
    # data
    # id
    train_id = get_data(params=params, train=True, category="id", trn="simclr", formatter="aug")
    ...
    train_oe = get_data(params=params, train=True, category="id", trn="simclr", formatter="aug")

    data_oe_samples = torch.utils.data.DistributedSampler(
        train_oe,
        num_replicas=world_size,
        rank=rank
    )
    ...
    loader_oe = torch.utils.data.DataLoader(dataset=train_oe, batch_size=params["batch_size"],
                                            num_workers=params["num_workers"],
                                            pin_memory=True, sampler=data_oe_samples, drop_last=True)
    ...
    # run
    print(colored("Training protocol begins", "blue"))
    ...
    if rank == 0:
        ...
        # load data with validation transform.
        ...
        loader_val = {"id": loader_id_val, "ood": loader_ood_val}

        # load dara with simclr transform.
        id_clr = get_data(params=params, train=False, category="id", trn="simclr",
                          formatter=1)
        loader_id_clr = get_dataloader(params, id_clr, drop_last=False, shuffle=False)

        ood_clr = get_data(params=params, train=False, category="ood", trn="simclr",
                           formatter=1)
